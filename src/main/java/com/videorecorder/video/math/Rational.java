 ...
 */
package com.videorecorder.video.math;
...
public class Rational extends Number {
    ...
    public Rational multiply(Rational that) {
        if (Math.abs(this.num) < Integer.MAX_VALUE
                && Math.abs(this.den) < Integer.MAX_VALUE
                && Math.abs(that.num) < Integer.MAX_VALUE
                && Math.abs(that.den) < Integer.MAX_VALUE) {
    ...
    public Rational multiply(long integer) {
        if (integer==0) {
            ...
            return valueOf(
                    this.num,
                    this.den / integer);
        } else if (Math.abs(this.num) < Integer.MAX_VALUE
                && Math.abs(integer) < Integer.MAX_VALUE) {
            return valueOf(
                    this.num * integer,
                    this.den);
        } else {
    ...
    public static Rational valueOf(double d) {
        ...
        if (Double.isInfinite(d)) {
            return valueOf((long) Math.signum(d), 0);
        }
        if (Double.isNaN(d)) {
